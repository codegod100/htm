...
// node_modules/@atcute/tid/dist/s32.js
...
var createOAuthDatabase = ({ name }) => {
  ...
  return {
    ...
    dpopNonces: createStore("dpopNonces", (_item) => Date.now() + 10 * 60 * 1000)
  };
};
...
var getProtectedResourceMetadata = async (host) => {
  ...
  if (response.status !== 200 || extractContentType2(response.headers) !== "application/json") {
...
var isUseDpopNonceError = async (response, isAuthServer) => {
  ...
  if (isAuthServer === undefined || isAuthServer === true) {
    if (response.status === 400 && extractContentType2(response.headers) === "application/json") {
      try {
...
var finalizeAuthorization = async (params) => {
  ...
  if (!state2 || !(code || error)) {
    throw new LoginError(`missing parameters`);
  }
  const stored = database.states.get(state2);
  if (stored) {
    database.states.delete(state2);
  } else {
...
// lib.ts
async function authorizationUrl(handle, metadata) {
...
async function finalize(params, metadata) {
  configureOAuth({ metadata });
...
async function listRecords(repo, collection) {
  ...
  const { data } = await rpc2.get("com.atproto.repo.listRecords", {
    params: {
      repo,
      collection
    }
...
async function resolveHandle2(handle) {
  const manager = new CredentialManager({
    service: "https://bsky.social"
  });
  const rpc2 = new XRPC({ handler: manager });
  const { data } = await rpc2.get("com.atproto.identity.resolveHandle", {
    params: {
      handle
    }
  });
...
function cdnImage(did, link) {
  return `https://cdn.bsky.app/img/feed_thumbnail/plain/${did}/${link}`;
}
async function UrlPreview(url) {
  const resp = await fetch(`https://cardyb.bsky.app/v1/extract?url=${url}`).then((r) => r.json());
  return resp;
}
...
async function post({ text, metadata, image, handle }) {
  configureOAuth({ metadata });
  ...
  if (image && image.size !== 0) {
    const resp = await rpc2.call("com.atproto.repo.uploadBlob", {
      data: image
    });
    console.log({ resp });
    imageRecord = {
  ...
  await rpc2.call("com.atproto.repo.putRecord", {
    data: {
...
async function getCards(repo) {
  const did = await resolveHandle2(repo).then((r) => r.did);
...
// app.ts
var meta2 = await fetch("/client-metadata.json").then((r) => r.json());
...
class MyElement extends LitElement {
  ...
  async _update() {
    ...
    const url = await authorizationUrl(this._input.value, meta2);
    location.assign(url);
  }
  async _login() {
  }
...
class OtherElement extends LitElement {
  ...
  _task = new Task(this, {
    task: async ([], { signal }) => {
      console.log("loading");
      const session = await finalize(this.params, meta2);
      console.log({ session });
      return session;
    },
    args: () => []
...
//# debugId=4571AF514106E58064756E2164756E21
